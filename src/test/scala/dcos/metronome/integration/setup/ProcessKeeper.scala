...
*/
  ...
  def setupMesosEnv(workDirFile: File, workDir: String, containerizers: String = "mesos") = {
  ...
  def startMetronome(cwd: File, env: Map[String, String], arguments: List[String],
                     mainClass:   String = "dcos.metronome.JobApplicationLoader",
                     startupLine: String = "Started ServerConnector",
                     processName: String = "metronome"): Process = {
    ...
    val metronomeWorkDir: String = s"/tmp/metronome-itest-$processName"
    val metronomeWorkDirFile: File = new File(metronomeWorkDir)
    FileUtils.deleteDirectory(metronomeWorkDirFile)
    FileUtils.forceMkdir(metronomeWorkDirFile)

    val secretPath = write(metronomeWorkDirFile, fileName = "metronome-secret", content = "secret1")
    ...
    startJavaProcess(
      ...
      env + (ENV_MESOS_WORK_DIR -> metronomeWorkDir),
